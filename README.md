...
├── tests/
├── work/                  # local diagnostics; excluded from Git
├── requirements.txt
├── pytest.ini
└── README.md
```
...
**AI-Assisted Equity Research Platform | Python, Streamlit, Ollama, LLMs**

- Built a local-first equity-research platform that combines deterministic financial analysis with structured local-LLM investment interpretation and an independent AI risk review.
- Implemented evidence-constrained prompting, Pydantic schema validation, deterministic citation checks, and a publication gate to identify unsupported AI claims and overconfident recommendations.